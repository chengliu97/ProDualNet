...
class DecLayer(nn.Module):
    def __init__(self, num_hidden, num_in, dropout=0.1, num_heads=None, scale=30):
        ...
        self.W1 = nn.Linear(num_hidden  + num_in, num_hidden, bias=True)
        self.W11 = nn.Linear(num_hidden + num_hidden + num_in, num_hidden, bias=True)
        self.fc1 = nn.Linear(num_hidden , num_hidden, bias=True)
    ...
    def forward(self, h_V, h_E, mask_V=None, mask_attend=None):
        ...
        h_V_1 = h_V.reshape(B//2, 2, L, 128)
        

 
        h_V_1 = torch.cat([h_V_1[:,1].unsqueeze(1), h_V_1[:,0].unsqueeze(1)],1)


        h_V_1 = self.fc1(h_V_1.reshape(B, L, 128)*mask_V.unsqueeze(-1))

        h_V_expand_1 = h_V_1.unsqueeze(-2).expand(-1,-1,h_E.size(-2),-1)


        h_EV = torch.cat([h_V_expand, h_E], -1)

        h_EV = torch.cat([h_EV, h_V_expand_1], -1)


        h_message = self.W3(self.act(self.W2(self.act(self.W11(h_EV)))))
        if mask_attend is not None:
            h_message = mask_attend.unsqueeze(-1) * h_message
...
class ProDualNet(nn.Module):
    def __init__(self, num_letters=21, node_features=128, edge_features=128,
        ...
        #self.W_out_1 = nn.Linear(hidden_dim*2, num_letters, bias=True)
        self.W_out_1 = nn.Linear(hidden_dim*2, num_letters)
    ...
    def forward(self, X, S,S_embed,mask_train, mask, chain_M, residue_idx, chain_encoding_all,  is_eval = False ):
        ...
        if is_eval:
            ...
            B1,N1 = chain_M.shape
            chain_M = chain_M.reshape(B1//2,2,N1)
            chain_M_com = chain_M[:,0]*chain_M[:,1]
            decoding_order = torch.argsort((chain_M_com+0.0001)*(torch.abs(torch.randn(chain_M_com.shape, device=device)))) #[numbers will be smaller for places where chain_M = 0.0 and higher for places where chain_M = 1.0]
            #print(decoding_order.shape)
            decoding_order = decoding_order[:,None].repeat(1,2,1)
            decoding_order = torch.flatten(decoding_order, start_dim=0, end_dim=1)
            mask_size = E_idx.shape[1]
            permutation_matrix_reverse = torch.nn.functional.one_hot(decoding_order, num_classes=mask_size).float()
            order_mask_backward = torch.einsum('ij, biq, bjp->bqp',(1-torch.triu(torch.ones(mask_size,mask_size, device=device))), permutation_matrix_reverse, permutation_matrix_reverse)
        ...
        #h_V B L 128
        h_V = h_V.reshape(B//2, 2, L, 128)

 
        h_V = h_V.permute(0, 2, 1, 3)


        h_V = h_V.reshape(B//2, L, 256)
        logits = self.W_out_1(h_V)
        ...
        for t_ in range(N_nodes):
            ...
            if (mask_gathered==0).all(): #for padded or missing regions only
                ...
                if (chain_M_pos*chain_mask)[0,t[0]] == 1:
                    ...
                    h_V_t = h_V_t.reshape(1, 256).repeat(2,1)
                    logits = self.W_out_1(h_V_t)/ temperature
