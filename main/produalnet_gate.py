...
class ProDualNet(nn.Module):
    def __init__(self, num_letters=21, node_features=128, edge_features=128,
        ...
        self.W_out_1 = nn.Linear(hidden_dim, num_letters, bias=True)
    ...
    def forward(self, X, S,S_embed,mask_train, mask, chain_M, residue_idx, chain_encoding_all,  is_eval = False ):
        ...
        h_E_flat = h_E.mean(-2) #B,L,128 
        h_E_flat = h_E_flat.reshape(B//2, 2, L, 128)

        g1 = torch.cat([h_E_flat[:,0],h_V[:,0],h_E_flat[:,1]],-1)
        g2 = torch.cat([h_E_flat[:,1],h_V[:,1],h_E_flat[:,0]],-1)
        g1 = self.gate(g1)#b,l,1
        g2 = self.gate(g2)#b,l,1


        logits = self.W_out_1(g1*self.mlp1(h_V[:,0])+g2*self.mlp1(h_V[:,1]))
        ...
        for t_ in range(N_nodes):
            ...
            if (mask_gathered==0).all(): #for padded or missing regions only
                ...
                if (chain_M_pos*chain_mask)[0,t[0]] == 1:
                    ...
                    #h_V_t 2,128
                    h_E_flat = h_E.mean(-2) #B,L,128 
                    B,L = h_E_flat.shape[:2]
                    h_E_flat = h_E_flat.reshape(B//2, 2, L, 128)
                    h_E_flat = h_E_flat[:,:,t[0]].unsqueeze(-2)#1,2,1,128
                    #h_E_flat[:,0] 1 1 128 h_V_t[None,None,0] 1,1,128
                    #print(h_E_flat.shape,h_E_flat[:,0].shape,h_V_t[None,None,0].shape)
                    g1 = torch.cat([h_E_flat[:,0],h_V_t[None,None,0],h_E_flat[:,1]],-1)
                    g2 = torch.cat([h_E_flat[:,1],h_V_t[None,None,1],h_E_flat[:,0]],-1)
                    g1 = self.gate(g1)#b,1,1
                    g2 = self.gate(g2)#b,1,1
                    logits = self.W_out_1(g1*self.mlp1(h_V_t[None,None,0])+g2*self.mlp1(h_V_t[None,None,1]))
                    #1,1,21
                    logits = logits[0].repeat(2,1)/ temperature
